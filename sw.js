...
self.addEventListener('fetch', e => {
    ...
    const res = caches.match(e.request).then(res => {
        ...
        .then(res_net => {
            ...
            if (e.request.headers.get('accept').includes('text/html')) return caches.match('/pages/view_offline.html')
            if (e.request.url.includes('.jpeg') || e.request.url.includes('.jpg') || e.request.url.includes('.png')) return caches.match('/img/default.jpg')
        })
    })
    e.respondWith(res)
